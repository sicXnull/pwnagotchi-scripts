...
def generateHashcatScript(filename):
    ...
    for attack in attacks:
        ...
        if("0" in attack[0]):
            if (("ssid-ninja.rule" in attack[1]) or ("4-digit-append.rule" in attack[1])):
                hashcatCommand = 'python "' + wordNinjaPath + '/wordNinjaGenerator.py" ' + ' | ' + hashcatCommand
...
def generateScriptForBatch():
    ...
    if(len(sessionScripts) > 1):
        f = open(f"./hashcat/scripts/batches/batch-{randint(1000, 9999)}.sh", "w")
        batchScript = ""
    ...
    def generateScriptsForHC():
        global hashcatScriptVersion
        for folder in ["hash"]:
            for filename in os.listdir("./handshakes/hash" + folder):
